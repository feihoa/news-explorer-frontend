...
document
  .querySelector("#two-lines")
  .addEventListener("click", handleMenuIconLinesClick);
document
  .querySelector("#button-logout")
  .addEventListener("click", handleLogout, { once: true });
...
function getMainArticlesInfo() {
  return mainApi
    ...
    .then((data) => {
      ...
      if (
        (data.data.length > 4 && data.data.length < 21) ||
        String(data.data.length).slice(-1) > 4
      ) {
        articlesNumber.textContent = `${data.data.length} сохраненных статей`;
      } else if (
        (data.data.length > 1 && data.data.length < 5) ||
        (String(data.data.length).slice(-1) > 1 &&
          String(data.data.length).slice(-1) < 5)
      ) {
      ...
      if (keywords.length > 3) {
        keywordsElem.textContent = `По ключевым словам: ${keywords[0]}, ${
          keywords[1]
        }  и ${keywords.length - 2} другим`;
