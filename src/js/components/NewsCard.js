...
export class NewsCard extends BaseComponent {
      ...
      data.filter(function (elem) {
        if (elem.link === card.href) {
          remover(elem._id)
            .then((data) => {
              if (data && data != "TypeError: Failed to fetch") {
                document.querySelector("#card-zone").removeChild(card);
              }
            })
            .catch((err) => {
              console.log(err);
  ...
  create(
    ...
    if (window.location.pathname.includes("/articles.html")) {
      key = `<span  id="keyWord" class="text news-card__key-word">${this._sanitizeHTML(
        keyword
      )}</span>`;
    ...
    return ` <a class="news-card" target="_blank" href="${this._sanitizeHTML(
      newsUrl
    )}">
  <div class="news-card__image" style="background-image: url(${this._sanitizeHTML(
    cardImageValue
  )})">
      ${key}
      ${icon}
  ...
  <div class="news-card__content">
    ...
    <p class="text text_black news-card__text">${this._sanitizeHTML(
      cardDescriptionValue
    )}</p>
  </div>
  <p class="title news-card__source">${this._sanitizeHTML(
    cardSourceNameValue
  )}</p>
</a>
`;
