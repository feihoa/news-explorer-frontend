...
export class Form extends BaseComponent {
  ...
  _showErrorMessage(element) {
    ...
    if (element.value.length === 0) {
      ...
      element.type === "password" &&
      (element.value.length < 8 || element.value.length > 30)
    ) {
  ...
  _validateForm(button) {
    ...
    const array = [];
    Array.from(
      document.forms.popupForm.querySelectorAll("input")
    ).forEach((elem) => array.push(elem.value));

    if (
      array.every((elem) => elem.length > 1 && elem.length < 31) &&
      this._isEmail(array[0]) &&
      array[1].length >= 8
    ) {
      button.removeAttribute("disabled");
    } else {
